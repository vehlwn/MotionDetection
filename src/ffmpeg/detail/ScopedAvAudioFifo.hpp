...
public:
    ...
    ScopedAvAudioFifo& operator=(const ScopedAvAudioFifo&) = delete;
    ScopedAvAudioFifo& operator=(ScopedAvAudioFifo&& rhs) noexcept
    {
        swap(rhs);
        return *this;
    }
    ...
    void add_samples(std::uint8_t** const data, const int frame_size) const
    {
        ...
        errnum = av_audio_fifo_write(
            m_raw,
            reinterpret_cast<void**>(data), // NOLINT: cast from uint8_t to void
            frame_size);
        if(errnum < 0) {
            throw ErrorWithContext(
